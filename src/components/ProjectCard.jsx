...
const ProjectCard = ({ title, subtitle, image, technologies, href }) => {
    return (
        <motion.div
            ...
            className="bg-deep-blue/95 backdrop-blur-sm rounded-xl shadow-2xl overflow-hidden hover:shadow-blue/30 hover:shadow-2xl transition-all duration-300 hover:-translate-y-2 w-full md:w-96 lg:w-[26rem] border-2 border-blue/30 hover:border-blue/60 group"
        >
            <a href={href} target="_blank" rel="noopener noreferrer" className="block">
                {/* Project Image - Reduced height for wider appearance */}
                <div className="relative h-[15rem] sm:h-60 md:h-48 overflow-hidden">
                    <img
                        src={image}
                        alt={title}
                        className="w-full h-full p-3 rounded-3xl object-cover transition-transform duration-300 group-hover:scale-105"
                    />
