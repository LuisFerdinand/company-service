...
const Link = ({
  ...
  return (
    <button
      onClick={handleClick}
      className={`group relative flex items-center gap-2 px-2 sm:px-3 md:px-4 py-2 rounded-xl font-opensans text-xs sm:text-sm font-medium transition-all duration-300 ${
        selectedPage === lowerCasePage || location === `/${lowerCasePage}`
    ...
    >
      ...
      <span className="relative flex whitespace-nowrap justify-between w-full">
        ...
        {!isMobile && (
          <span
            className={`absolute -bottom-1 left-0 h-0.5 bg-gradient-to-r from-blue to-purple transition-all duration-300 ${
              selectedPage === lowerCasePage || location === `/${lowerCasePage}`
                ? "w-full"
...
const Navbar = ({ isTopOfPage, selectedPage, setSelectedPage }) => {
  ...
  return (
    <nav
      className={`${navbarStyle} fixed top-0 w-full z-20 transition-all duration-500`}
    >
      ...
      {showMobileMenu && (
        <>
          {/* Backdrop */}
          <div
            className={`z-30 fixed inset-0 bg-black/50 backdrop-blur-sm transition-opacity duration-300 ${
              isMenuOpen
          ...
          <div
            className={`z-40 fixed top-0 right-0 h-full w-72 max-w-[85vw] bg-deep-blue/95 backdrop-blur-xl border-l border-white/10 shadow-2xl transform transition-transform duration-500 ease-out ${
            ...
            <div className="p-4 sm:p-6 border-b border-white/10">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="font-inter text-base sm:text-lg font-bold text-white">
                      Digital
                    </h2>
                    <span className="font-opensans text-xs text-blue font-medium tracking-widest uppercase">
                      Solutions
      ...
      <div
        className={`absolute bottom-0 left-0 h-0.5 bg-gradient-to-r from-blue via-purple to-yellow transition-all duration-1000 ${
...
const NavDropdown = ({
  ...
  return (
    <div className="relative group">
      ...
      <div className="absolute top-full mt-2 w-48 bg-deep-blue border border-white/10 rounded-xl shadow-lg opacity-0 group-hover:opacity-100 scale-95 group-hover:scale-100 transition-all duration-200 z-20">
        {items.map((sub, index) => (
          <button
            key={index}
            onClick={() => handleNavigate(sub.href)}
            className="cursor-pointer block px-4 py-2 text-sm text-white hover:bg-white/10 transition w-full items-start text-start"
          >
            {sub.label}
          </button>
        ))}
